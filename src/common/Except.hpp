...
class Except : public std::exception {
    static const std::unordered_map<cl_int, const char*> s_err2Str;

    cl_int m_error;
public:
    Except(cl_int err) noexcept : std::exception(), m_error(err) {}

    const char* what() const noexcept override {
        return s_err2Str.at(m_error);
    }

    cl_int err() const noexcept {
        return m_error;
    }
};

inline const std::unordered_map<cl_int, const char*> Except::s_err2Str{
